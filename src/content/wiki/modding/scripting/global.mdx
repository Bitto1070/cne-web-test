...
There is only a few functions that `GlobalScript` manages itself, though you can call custom functions yourself from other scripts by doing <InlineCode lang='haxe' code={'GlobalScript.call("functionName", [/* arguments */])'}/>.<br/>
...
`GlobalScript` maintains it's own `ScriptPack` that is <b>_freely accessible_</b> as <InlineCode lang='haxe' code={`GlobalScript.scripts`}/> as a `static` variable.

You can manage the whole `ScriptPack` anywhere in your mod, but for simplicity you should use <InlineCode lang='haxe' code={`GlobalScript.scripts`}/> only if you need to access either a specific script inside, or internal variables. Using the `public` modifier in `GlobalScript` should never be used, as it only allows other `GlobalScripts` to access it, so it does nothing for your mod.

You can either make `static` variables / functions inside your `GlobalScript` to call anywhere, or just normal functions you can then call with <InlineCode lang='haxe' code={`GlobalScript.call()`}/>.
...
or... you can just straight up call them without <InlineCode lang='haxe' code={`GlobalScript.scripts.call()`}/>!
<div style="display: grid; justify-content: center;">
