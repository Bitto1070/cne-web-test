...
- The non-regular vector families (<code class="token keyword" set:html="ivecX"/>, <code class="token keyword" set:html="bvecX"/>, <code class="token keyword" set:html="uvecX"/>), and <InlineCode lang="glsl" code={`uint`}/> are all unsupported on certain platforms, so avoid using them.
