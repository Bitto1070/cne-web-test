...
function typeHtml(t) {
	...
	return esc(t).replace(/\b([A-Z]\w*)\b/g, (m, name) => {
		if (KNOWN.has(name)) return `<a class="type" href="${name}">${name}</a>`;
...
function page(title, sidebar, content) {
	const nav = `<input class="search" type="text" placeholder="Filter classes...">` + sidebar;
	const main = content + `<footer>generated by hxdoc // hxdoc made by HeroEyad</footer>`;
	return `---
import Page from '${PAGE_IMPORT}';
const nav = ${JSON.stringify(nav)};
const main = ${JSON.stringify(main)};
const sidebarJs = ${JSON.stringify(SIDEBAR_JS)};
---
<Page title="API - ${esc(title)}">
	<div class="panel" style="width: fit-content; margin: auto;">
		<nav set:html={nav}></nav>
		<main set:html={main}></main>
	</div>
	<script is:inline set:html={sidebarJs}></script>
</Page>
`;
}
...
function buildSidebar(classes, activeName) {
	...
	for (const pkg of Object.keys(byPkg).sort()) {
		...
		for (const c of byPkg[pkg].sort((a, b) => a.name.localeCompare(b.name))) {
			html += `<li data-name="${esc(c.name)}"><a href="${c.name}"${c.name === activeName ? ' class="active"' : ""}><span class="k">${c.kind[0].toUpperCase()}</span>${esc(c.name)}</a>`;
			if (c.fields.length) {
				html += `<ul class="mem">`;
				for (const f of c.fields) html += `<li data-name="${esc(f.name)}"><a href="${c.name}#${esc(f.name)}">${esc(f.name)}</a></li>`;
				html += `</ul>`;
			}
			html += `</li>`;
		}
...
function buildIndex(classes, sidebar) {
	...
	for (const c of classes.sort((a, b) => a.name.localeCompare(b.name))) {
		...
		body += `<li><a href="${c.name}">${esc(c.name)}</a><div class="d">${esc(summary)}</div></li>`;
	}
	body += `</ul>`;
...
for (const cls of allClasses) {
	...
	fs.writeFileSync(path.join(OUT, cls.name + ".astro"), buildClassPage(cls, sidebar));
}
